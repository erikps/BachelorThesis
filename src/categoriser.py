...
class HCategoriser(Categoriser):
    ...
    def __call__(self, framework: WeightedArgumentationFramework, use_predicted_edges=True):
        """
        ...
        while mean_squared_delta is None or mean_squared_delta > self.stop_delta:
            ...
            predicted_edges = networkx.get_edge_attributes(framework.graph, "predicted_edge")
            ...
            for node, data in nodes:
                def is_relevant_attacker(attacker):
                    ...
                    return predicted_edges[(attacker, node)] if use_predicted_edges \
                        else actual_edges[(attacker, node)]

                # first, accumulate all of the node's attacker's weights
                attacker_degrees = [nodes[attacker]["predicted_degree"] for attacker in framework.graph.predecessors(node) if is_relevant_attacker(attacker)]
            ...
            for node, data in nodes:
                squared_delta = (
                    data["predicted_degree"] - data["new_predicted_degree"]) ** 2
