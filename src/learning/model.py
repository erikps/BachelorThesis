...
class DeepQAgent:
    ...
    def learn(self, experience: ExperienceType) -> Dict[str, Any]:
        ...
        q_nexts = torch.stack([self._online_model(state).squeeze() for state in next_states])
    ...
    def get_action(self, data: Data, do_exploration: bool = True) -> int:
        ...
        q_values = self._online_model.readout(data)
...
if __name__ == "__main__":
    ...
    # try:
    #     agent, model_path = DeepQAgent.load_snapshot(AGENT_NAME)
    ...
    # dataset = AttackInferenceDataset(problems_directory="./data/train/train-4/attackinference")
