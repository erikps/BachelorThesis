...
class TestVerifier(unittest.TestCase):
    ...
    def test_example_with_swapped_attacks(self):
        ...
        def check(node: str, value: float):
            predicted = example_problem.framework.graph.nodes(data=True)[node]["predicted_degree"]
        ...
        self.assertAlmostEqual(float(self.verifier.evaluate(example_problem)), 0.01175336072, places=6)
        self.assertFalse(self.verifier(exasmple_problem))
